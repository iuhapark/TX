...
export default function AuthPage() {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8080/auth/url")
      .then((response) => {
    ...
    if (code) {
      axios
        .get(`http://localhost:8080/auth/callback?code=${code}`)
        .then((response) => {
          const { token } = response.data;
          setCookie(null, "accessToken", token, { path: "/" });
          setComponentToShow("protected");
        })
        .catch((error) => {
    ...
    if (token) {
      axios
        .get("http://localhost:8080/auth/messages", {
          headers: {
    ...
    <div className="flex justify-center h-screen w-full px-5 sm:px-0">
      <div className="mt-28 w-[73vh] h-[67vh] flex justify-center items-center shadow-none">
        <div className="w-4/5 p-[8.5vh]">
          <h1 className="flex justify-center">안녕하쎄용! {content}</h1>
          <div>
            {" "}
            <a
              href={url}
              className="AuthButton btn btn-sm btn-success active"
              role="button"
            >
              <div className="flex px-5 justify-center w-full py-4">
                ...
                <div className="flex w-full justify-center">
                  <h1 className="whitespace-nowrap text-[15px]">
                    contine with google
                  </h1>
                </div>
              </div>
            ...
            <a
              href={`${PG.USER}/login`}
              className="AuthButton flex px-5 justify-center w-full py-4"
            >
              <div className="flex w-full justify-center">
